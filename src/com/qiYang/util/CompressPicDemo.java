 ...
 */  
 public class CompressPicDemo {  
	 private static Logger logger=Logger.getLogger(CompressPicDemo.class);
    ...
    /* private String inputFileName; // 输入图文件名  
     ...
     private int minSize= 20; // 执行压缩条件，图片大于多少时。 默认200KB
...
//     }   
         ...
         try {   
        	/* File inf = new File(inputDir);
             ...
             if (img.getWidth(null) == -1) {  
                 System.out.println("无法读取,请重试!" + "<BR>");   
                 return false;   
             } else {   
                 int newWidth; int newHeight;   
                 if((file.length()/1024)>minSize){
    ...
    public Boolean compressPic (String inputDir, String outputDir,int minSize) {   
        // 输入图路径   
        this.inputDir = inputDir;   
        // 输出图路径   
        this.outputDir = outputDir;   
    ...
    public Boolean compressPic(String inputDir, String outputDir, int width, int height,int minSize, boolean gp) {   
        // 输入图路径   
        this.inputDir = inputDir;   
        // 输出图路径   
        this.outputDir = outputDir;   
    ...
    public static void main(String[] arg) {   
        ...
        for (int i = 0; i < 3; i++) {   
            ...
            mypic.compressPic("e:\\1.jpg", "e:\\test\\c1"+i+".jpg" , 300, 300,20, true);   
            int end = (int) System.currentTimeMillis(); // 结束时间   
            int re = end-start; // 但图片生成处理时间   
            count += re; System.out.println("第" + (i+1) + "张图片压缩处理使用了: " + re + "毫秒");   
            System.out.println("输出的图片大小：" + mypic.getPicSize("e:\\test\\c1"+i+".jpg")/1024 + "KB");   
        }  
