...
<html>
  ...
  <body>
    <center>
    	<h3>课程</h3>
    <s:form action="selectaction!updateCourse.action" name="myform" method="post" theme="simple">
    	<table>
    		<s:hidden name="tbCourse.courseId"></s:hidden>
    		<tr>
    			<td>城区</td>
    			<td>
    				<select id="town" onchange="seltown()" name="tbCourse.tbTown.townId">
	   					...
	   					<s:iterator value="listtown">
	   						<s:if test="tbCourse.tbTown.townId==townId">
	   							<option value='<s:property value="townId"/>' selected="selected"><s:property value="townName"/></option>
	   						</s:if>
	   						<s:else>
	   			...
	   			<td>
   					<select id="branchSchool" onchange="selgrade()" name="tbCourse.tbBranchschool.branchSchoolId">
	   					...
	   					<s:iterator value="listbranchschool">
	   						<s:if test="tbCourse.tbBranchschool.branchSchoolId==branchSchoolId">
	   							<option value='<s:property value="branchSchoolId"/>' selected="selected"><s:property value="braschName"/></option>
	   						</s:if>
	   						<s:else>
	   							<option value='<s:property value="branchSchoolId"/>'><s:property value="braschName"/></option>
	   						</s:else>
	   					</s:iterator>
	   				</select>
		   		</td>
	   		...
	   		<tr>
	   			...
	   			<td>
   					<select id="grade" name="tbCourse.tbGrade.gradeId">
	   					...
	   					<s:iterator value="listgrade">
	   						<s:if test="tbCourse.tbGrade.gradeId==gradeId">
	   			...
	   			<td>科目</td>
	   			<td><s:select list="listSubject" listKey="subjectId" listValue="subjectName" name="tbCourse.tbSubject.subjectId" headerKey="" headerValue="--请选择--"></s:select></td>
	   		</tr>
	   		<tr>
	   			<td>课程</td>
	   			...
	   			<td>课程成绩</td>
	   			<td>
	   				<s:radio list="#{'0':'不限','1':'优','2':'良','3':'中','4':'差'}" name="tbCourse.courseScore" id="isGood"></s:radio>
	   			</td>
	   		</tr>
	   		<tr>
	   			<td>是否推荐</td>
	   			<td>
	   			<s:radio list="#{'0':'是','1':'否'}" name="tbCourse.isGood" id="isGood"></s:radio>
	   			</td>
	   			<td>课程介绍</td>
